...
def _add_list_group_spacing(text, entities):
    """Insert blank line after sub-item blocks when followed by other content."""
    lines = text.split("\n")
    if len(lines) <= 1:
    ...
    for i, line in enumerate(lines):
        ...
        if i < len(lines) - 1:
            ...
            next_is_subitem = next_line.strip().startswith("\u2981")
            if is_subitem and not next_is_empty and not next_is_subitem:
